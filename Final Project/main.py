    ...
    for run in range(3):
        ...
        while T > 0.000001:
            ...
            for i in range(5):
                ...
                elif math.exp((get_my_acc(tmp_neighbor) - get_my_acc(neighbor)) / T) > rd.random():
                    tmp_neighbor = neighbor
            tmp_bst = tmp_neighbor
            if get_my_acc(best_acc_array) <= get_my_acc(tmp_bst) and get_my_latency(tmp_bst) < 60:
                best_acc_array = tmp_bst
            elif get_my_latency(best_acc_array) > get_my_latency(tmp_bst) and get_my_latency(best_acc_array) > 60:
                best_acc_array = tmp_bst
